...
const [histogramData, setHistogramData] = useState<{ name: string, count: number, sortKey: number }[]>([]);
...
// Fetch Histogram Data (Full Dataset)
useEffect(() => {
  if (loading) return;
  try {
    const hist = dbService.getHistogram(filters);
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100 relative">
        <h3 className="font-bold text-slate-800 mb-6 flex items-center gap-2">
          ...
          <div className="group relative">
            ...
            <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 p-2 bg-slate-800 text-white text-xs rounded shadow-lg w-48 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
              Shows distribution of all matching properties (truncated at 2000% increase).
            </div>
          </div>
        </h3>
        <div className="h-64" style={{ height: '16rem' }}>
